...
abstract: >
  ...
  本文利用 Gaia DR3 参考星、HEALPix 全天采样和传播至 J2026.0 的位置协方差，
  在具有代表性的约一度稀疏视场条件下，评估九种多项式板模型的板解不确定度。
  对于所采用的 G < 10 等参考星样本，四参数线性模型具有最低的板常数方差，
  对应位置不确定度的中位数为0.95 mas，第95百分位数为1.7 mas。
  但较低的方差并不意味着该模型能够充分改正非线性光学畸变。
  根据一级近似关系换算，相关角度不确定度对应约0.9–2.5 m/s的速度误差尺度。
  研究表明，在 StarNAV 误差预算和导航滤波器中，应显式考虑板解不确定度与残余畸变偏差。

summary: "通过九种板模型的全天评估，表明板解不确定度可对恒星光行差自主导航产生米每秒量级的速度误差贡献。"
